...
the only lane touching `jack_freeman_catalog.tech_summit_scada_build` and the Lakebase project. This is the lane with
...
Use profile `<PROFILE>`. Prefix every pipeline and job you create with `a_`, and
prefix any scratch tables `scratch_a_`. **Everything lives in the one schema `jack_freeman_catalog.tech_summit_scada_build`, so there is
no schema boundary protecting anyone.** Read the table ownership map in
`contract/naming.md` and stay strictly inside your rows of it. Never touch the four frozen `dim_*`
tables, and never create or replace lane B's or lane C's tables.

**Step 1, do this before anything else, my colleagues are blocked until it lands.**
Write a synthetic data generator driven entirely by `jack_freeman_catalog.tech_summit_scada_build.dim_tag`, so adding
a tag to the register adds it to the output with no code change. Use it to populate
both seed tables:

* `jack_freeman_catalog.tech_summit_scada_build.tag_reading_seed`: 24 hours, full rate, every one of the 121 tags,
...
* `jack_freeman_catalog.tech_summit_scada_build.vibration_features_seed`: 120 days at 10 minute grain, with the
...
* **Q1**, all readings into Lakebase:
  ...
  (df.writeStream
     .format("postgresql")
     .outputMode("update")
     .option("checkpointLocation", "/Volumes/jack_freeman_catalog/tech_summit_scada_build/landing/checkpoints/tag_current")
...
* **Q2**, rule failures only. Evaluate `jack_freeman_catalog.tech_summit_scada_build.dim_rule` as a broadcast
...
**Step 4.** Start Lakebase CDF from the `plant` schema to `jack_freeman_catalog.tech_summit_scada_build`. Confirm with
...
* `jack_freeman_catalog.tech_summit_scada_build.lb_tag_current_history` is populating, allowing for the roughly 15
...
**Step 5, seam 1.** Repoint `jack_freeman_catalog.tech_summit_scada_build.vw_tag_reading` at the live path
...
Guardrails. Do not use `CREATE OR REPLACE` on anything in `jack_freeman_catalog.tech_summit_scada_build`, it is
...
that fallback is a bronze streaming table at `jack_freeman_catalog.tech_summit_scada_build.tag_reading` with
